...
export function CardArt({ track, isActive }: CardArtProps) {
  return (
    <div className="relative w-full h-full rounded-2xl overflow-hidden select-none bg-neutral-900 group">
      ...
      {isActive && (
        ...
        >
          ...
          <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-full border border-white/10 flex items-center justify-center">
            <div className="w-16 h-16 sm:w-18 sm:h-18 rounded-full border border-white/10 flex items-center justify-center">
              {/* Vinyl Center Label */}
              <div
                className="w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center shadow-inner text-[8px] font-bold text-white text-center p-0.5 border border-white/30"
                style={{ backgroundColor: track.accentColor }}
              >
                <div className="w-2.5 h-2.5 rounded-full bg-stone-950 border border-white/40" />
              </div>
            </div>
          </div>
        </motion.div>
